...
namespace HuffmanEncode
{
    ...
    class Program
    {
        ...
        static void Main()
        {
            ...
            // Create a list of characters, frequencies, and probabilities
            var frequency = BuildFrequency(text1);
            var totalSymbols = text1.Length;
            var characters = frequency.Keys.ToList();
            var frequencies = frequency.Values.ToList();
            var probabilities = frequencies.Select(freq => (double)freq / totalSymbols).ToList();

            // Combine data into a list of tuples for sorting
            var data = characters.Zip(frequencies, (c, freq) => (Character: c, Frequency: freq))
                                .Zip(probabilities, (tuple, prob) => (tuple.Character, tuple.Frequency, Probability: prob))
                                .ToList();

            // Sort the data by probability in descending order
            var sortedData = data.OrderByDescending(item => item.Probability).ToList();

            // Display sorted characters and probabilities
            foreach (var item in sortedData)
            {
                Console.WriteLine($"Character: {item.Character}, Huffman Code: {mapping[item.Character]}, Probability: {item.Probability}");
            }
